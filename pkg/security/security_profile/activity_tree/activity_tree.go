...
// Merge will merge the new tree into the base one
func (at *ActivityTree) Merge(newTree *ActivityTree) {
	// merge DNSNames
	for _, key := range newTree.DNSNames.Keys() {
		at.DNSNames.Insert(key)
	}

	// merge Syscalls
	for id := range newTree.SyscallsMask {
		at.SyscallsMask[id] = id
	}

	// merge root processes
	newProcesses := []*ProcessNode{}
	// loop on new tree root processes
	for _, newProcess := range newTree.ProcessNodes {
		alreadyPresent := false
		// search if it's already present or not
		for _, oldProcess := range at.ProcessNodes {
			// if yes, merge them
			if oldProcess.Matches(&newProcess.Process, false, true) {
				alreadyPresent = true
				oldProcess.merge(newProcess, at.pathsReducer)
				break
			}
		}
		// if not, add it
