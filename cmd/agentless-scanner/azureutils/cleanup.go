...
func CleanupScan(ctx context.Context, cfg Config, scan *types.ScanTask, waiter *ResourceWaiter, resourceID types.CloudID) error {
	...
	case types.ResourceTypeSnapshot:
		if err := cleanupScanSnapshot(ctx, cfg, scan, waiter, resourceID); err != nil {
...
func cleanupScanSnapshot(ctx context.Context, cfg Config, maybeScan *types.ScanTask, waiter *ResourceWaiter, snapshotCloudID types.CloudID) error {
